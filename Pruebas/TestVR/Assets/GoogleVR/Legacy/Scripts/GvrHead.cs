...
public class GvrHead : MonoBehaviour
{
	...
	CMovement m_CachedMovement;
	void Start()
	{
		m_CachedMovement = GameObject.Find( "Player" ).GetComponent<CMovement>();
	}
	...
	private void UpdateHead( )
	{
		...
		if ( trackRotation )
		{
			...
			Vector3 tmpEuler = rot.eulerAngles;
			//We cancel the y-axis rotation
			//Debug.Log(tmpRot.y + " the vlaue of tmpRot.y is: ");
			if ( Mathf.Abs( rot.y ) > 0.2f )
			{
				//Debug.Log("The value of tmpEuler before the change is: " + tmpEuler.y );
				//Debug.Log("The value of the transform.eulerAngles is: " + transform.eulerAngles.y );
				tmpEuler.y = transform.eulerAngles.y + ( tmpEuler.y * ( rot.y > 0f ? 0.04f : -0.02f ) );
				//Debug.Log("The value after the change is: " + tmpEuler);
			}
			else
			{
				tmpEuler.y = transform.eulerAngles.y;
			}
			...
			if ( target == null )
			{
				...
				//Debug.Log( tmpRot );
				//m_CachedMovement.SetRotationFromVR( tmpRot );
			}
			else
			{
				transform.eulerAngles = tmpEuler;
				//m_CachedMovement.SetRotationFromVR( tmpRot );
				Debug.LogError("WARNING, entered the else clause.");
			}
		}
